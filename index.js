...
// Функция генерации отчета (вынесена для переиспользования)
...
bot.start(async (ctx) => {
    await ctx.reply(
        `Привет, ${ctx.from.first_name}! 👋\n\nЯ твой финансовый помощник. Я записываю твои траты прямо в Google Таблицу.\n\n` +
...
// Кнопка и команда: Статистика (текст)
...
// Обработка текстовых сообщений (запись трат)
    ...
    if (match) {
        ...
        try {
            ...
            const replyMessage = await ctx.reply(
                `✅ Записано: ${amount} грн. на ${category}`,
            );
            ...
            try {
                ...
                console.error("Не удалось удалить сообщение пользователя:", e);
            }

            // Удаляем ответ бота через 3 секунды
            setTimeout(async () => {
                try {
                    ...
                    console.error("Не удалось удалить ответ бота:", e);
                }
            }, 3000);
        } catch (error) {
        ...
        const errorMsg = await ctx.reply(
            'Не понимаю тебя. Пожалуйста, используй формат: "Сумма Категория" (например: 500 кофе) или нажми на кнопку меню.',
        );
